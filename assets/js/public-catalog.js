(function () {
  ...
  function addProductToCart(product, quantity = 1) {
    ...
    store.addCartItem({
      ...
      size: isPhysical ? '' : 'M',
      quantity,
      previewImage: gallery[coverIndex] || product.catalogImage,
      previewViews: {
    ...
    window.dispatchEvent(new CustomEvent('ursoninhos-cart-changed'));
  }
  ...
  function bindCardEvents(products) {
    grid.querySelectorAll('.public-product-card').forEach((card) => {
      ...
      card.querySelectorAll('[data-action="details"]').forEach((button) => {
